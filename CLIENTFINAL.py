...
import numpy as Npy #import numpy for fixing RGB bug caused by difference in pygame and numpy
...
import msgpack as MessagePack#import mgspack for converting the video frame into serlialised bytes for transmission
...
PortNumberControl = 2006#port number to handle user keyboard/joystick inputs to send to server
...
SocketConnectionControl = socket.socket() #initalise the socket for sending control commands
...
#and establish the connection with the server via its IP
SocketConnectionControl.connect((serverIP, PortNumberControl))#attach the control socket to the given port
#and establish the connection with the server via its IP
...
dataSize = struct.calcsize("i") #calculate the size of each frame based on Q (64 bit integers)
...
def updateGraphs(index, xCoordinates): #update each of the 4 graphs 
    ...
    try:
        ...
        #receive byte stream from raspberry pi representing 
        # the sensor values

        SensorData = SensorBytes.decode("utf-8") 
        #convert the byte stream into string format
        #using the unicode byte conversion
...
while True:
    ...
    leftYAxis = controller.get_axis(1)#get the left joystick y axis 
    rightYAxis = controller.get_axis(3)#get the right joystick y axis


    joystickDirections = "L: " + str(round(leftYAxis, 2)) + "\tR: " + str(round(rightYAxis,2))
    #store the Left and Right Y axis joystick directions for transmission

    SocketConnectionControl.send(bytes(joystickDirections,"utf-8"))
    #transmit the joystick controller over converting each unicode character
    #into byte format
    ...
    while len(accumulatedBytes) < dataSize:
        #keep collecting incoming bytes to match
        # the expected size of the the frame
        accumulatedBytes += SocketConnectionCamera.recv(65536) 
    ...
    calculatedFrameSize = struct.unpack("i", lowerSizeBytes)[0] #use the lower bytes to determine the integer value representing the frame size
    ...
    while len(accumulatedBytes) < calculatedFrameSize: 
        #get the remaining bytes to make up the overall frame
        accumulatedBytes += SocketConnectionCamera.recv(65536) 
        #receive bytes to make up the frame
    ...
    voltageText = "battery: " + str(round(int(((voltageReading-minimumVoltage)/(maximumVoltage-minimumVoltage)) * 100) - 100,0)) + "%"

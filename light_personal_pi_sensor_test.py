...
i2cbus = SMBus(1) #set up the I2C bus using the 1st channel which is the one the I2C pins on the raspberry pi use
...
i2cbus.write_byte_data(i2caddress, writeAddress, setupData)#write the set up data to the light sensor

while True:
    data = i2cbus.read_word_data(i2caddress, readAddress)#read data using the light sensor
    
    light_conversion = round(data * (120000/65536), 2)#conversion of raw 16-bit data into light sensor reading rounded to 2 decimal places
    
    print(light_conversion, "lux")
